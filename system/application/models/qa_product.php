...
/**
 * Description of qa_product
 *
 * @author purelogics
 */
class qa_product extends Model {
  //put your code here
  function qa_product()
  {
    parent::Model();
  ...
  function getProduct($store_id , $offset = 0, $limit = 10, $all = false, $sort_column = 'id', $sort_order = 'asc')
  {
    $this->db->where('qa_store_id', $store_id);
  ...
  function getProductCount($store_id, $all = false, $filter_text = '')
  {
  ...
  {    
    $query = 'SELECT *
      ...
      INNER JOIN qa_post AS q ON p.id = q.qa_ref_id
  ...
  {    
    ...
    $this->db->join('qa_team_member as tm', 'tm.qa_user_id = qa_product.user_id AND tm.role = "creator"');
    $this->db->where('qa_store_id', $store_id);
