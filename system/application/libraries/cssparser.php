...
class cssparser
{
  ...
  function Add($key, $codestr)
  {
    ...
    if(count($codes) > 0) {
      foreach($codes as $code) {
        ...
        if($code){
          $tokens = explode(":",$code);
          $codekey = strtolower($tokens[0]);
          if(count($tokens) == 2)
            $codevalue = $tokens[1];
          else
          {
            unset($tokens[0]);
            $codevalue = join(':', $tokens);
          }
          
          //echo 'ACTUAL: '.$code."\n";
          //echo 'KEY: '.$codekey.' , CODE: '.$codevalue."\n";
          if(strlen($codekey) > 0) {
  ...
  function unsetAttr($key, $attributes)
  {    
    $key = strtolower(trim($key));
    foreach($attributes as $attribute){
      if(!trim($attribute)) continue;
      if(isset($this->css[$key][trim($attribute)]))
        unset($this->css[$key][trim($attribute)]);
    }
  }
