 ...
 * 
 */
class Widget_new extends Controller
{
  function Widget_new()
  {
    parent::Controller();
  ...
  function main($team_member_id = '', $store_id = '', $ref_id = '', $ref_type = '', $widget_user_id = '', $navigate_to_post_id = 0)
  {
    ...
    $encoded_params = parse_encoded_params();
    extract($encoded_params);
    ...
    if($widget_user_id)
    {
      $data['cid'] = $widget_user_id;
      
      $widget_user_email = trim($_REQUEST['e']);
    ...
    $data = array_merge($data, $this->widget_configuration->getWidgetSettings($team_member_id));

    //may_require_exit($not_configured, $data['appearance']);
    
    $data['store_url'] = get_store_dir_url($store_id);
    ...
    if($ref_type == 'product')
    {
      ...
      $data['questions'] = $this->post->postDetails(session_get_widget_user('user_id'), $prod_map_id, $ref_type, '', $offset, $this->config->item('WIDGET_QUESTION_PER_PAGE'));
    }
    else
    {
      if(trim($navigate_to_post_id))
      {
        $offset = $this->post->get_question_offset_for_category($navigate_to_post_id, $prod_map_id, $ref_type, $this->config->item('WIDGET_QUESTION_PER_PAGE'));
      }
      
      $data['questions'] = $this->post->getPostForProducts(session_get_widget_user('user_id'), $prod_map_id, $ref_type, '', $offset, $this->config->item('WIDGET_QUESTION_PER_PAGE'));
    }
    ...
    // pagination
    $data['pagination'] = $this->widget_pager->get_pagination($total_questions, $offset, $this->config->item('WIDGET_QUESTION_PER_PAGE'));
  ...
  function questions($session_key, $filter, $offset = 0, $search_text = '')
  {
    ...
    $filter = check_for_recent_tab_contents($this, $widget_data, $filter);
    
    if($widget_data['ref_type'] == 'product')
    {
      $data['questions'] = $this->post->postDetails($widget_data['user_id'], $widget_data['prod_map_id'], $widget_data['ref_type'], $filter, $offset, $this->config->item('WIDGET_QUESTION_PER_PAGE'), $search_text);
    }
    else
    {
      $data['questions'] = $this->post->getPostForProducts($widget_data['user_id'], $widget_data['prod_map_id'], $widget_data['ref_type'], $filter, $offset, $this->config->item('WIDGET_QUESTION_PER_PAGE'), $search_text);
    }
  ...
  function savePost($session_key)
  {
    ...
    $question_id = isset($_REQUEST['qaw_question_id']) ? $_REQUEST['qaw_question_id'] : 0;
    ...
    if($question)
    {
      ...
      $data['qa_title'] = parse_href_tags(trim($_REQUEST['qaw_text_qa']));
      ...
      if($type == 'answer')
      {
        $view['similarPost'] = $this->post->similarPosts($question->qa_ref_id, $question->qa_post_type, $question_id);
      }
      
      $view['thank_you'] = $this->widget_configuration->getOne($widget_data['team_member_id'], 'thank_you');

      $response = array(
        'similar' => $this->load->view('widget/_thankYou', $view, true)
      );
    }
  ...
  function saveSpam($session_key)
  {
    ...
    $data['post_id'] = trim($_REQUEST['qaw_post_id']);
    $data['description'] = trim($_REQUEST['qaw_issue']);
   ...
   * 
   */
  function browseStore($session_key, $type = '', $character = '')
  {
    $widget_data = get_complete_widget_session($session_key);
    may_require_exit('failure', ($widget_data['user_id']));
    ...
    if($type == "categories")
    {
      $data['rows'] = $this->category->getCategory($widget_data['store_id'], 0, 100, 'qa_category_name', 'asc', $character, true, true);
      $data['item_field'] = 'qa_category_name';
    }
    else if($type =="brands")
    {
      $data['rows'] = $this->brand->getBrand($widget_data['store_id'], 0, 100, 'qa_brand_name', 'asc', $character);
      $data['item_field'] = 'qa_brand_name';
    }
    else if($type == "products")
    {
      $data['rows'] = $this->product->getProduct($widget_data['store_id'], 0, 100, false, 'qa_product_title', 'asc', $character);
      $data['item_field'] = 'qa_product_title';
    }
    
    $response['html'] = $this->load->view('widget/browseStore', $data, true);
   ...
   * 
   */
  function autoSearch($session_key, $type = '', $term = '')
  {
    $widget_data = get_complete_widget_session($session_key);
    may_require_exit('failure', ($widget_data['user_id']));
    ...
    if(trim($term))
    {
      if($type == "categories")
      {
        $json = $this->category->getCategoryByStoreId($widget_data['store_id'], $term);
      }
      elseif($type == "brands")
      {      
        $json = $this->brand->getBrandsByStoreId($widget_data['store_id'], $term);
      }
      elseif($type == "products")
      {
        $json = $this->product->getProductsByStoreId($widget_data['store_id'], $term);
      }
    }
   ...
   * 
   */
  function subCategories($session_key, $category_id)
  {
    $widget_data = get_complete_widget_session($session_key);
    may_require_exit('failure', ($widget_data['user_id']));
    
    $data['category_id'] = $category_id;
    $data['rows'] = $this->category->getCategory($widget_data['store_id'], 0, 1000, 'qa_category_name', 'asc', '', true, true, $category_id);
    
    $response['html'] = $this->load->view('widget/subCategories', $data, true);
   ...
   * 
   */
  function contributorDetail($session_key, $contributor_id)
  {
    $widget_data = get_complete_widget_session($session_key);
    may_require_exit('failure', ($widget_data['user_id']));
    ...
    require_once APPPATH . 'config/custom/settings.php';
    $data['config'] = $this->widget_configuration->getOne($widget_data['team_member_id'], 'contributor', true);
    
    /*echo '<pre>';
    print_r($data['config']);
    echo '</pre>';*/
    
    $response['html'] = $this->load->view('widget/contributorDetail', $data, true);
