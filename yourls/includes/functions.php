...
function yourls_insert_link_in_db( $url, $keyword, $title = '', $action_type = '', $store_id = 0, $item_id = 0, $item_type = '', $user_id = 0 ) {
	global $ydb;
	...
	$ip = yourls_get_IP();
	$insert = $ydb->query("INSERT INTO `$table` VALUES('$keyword', '$url', '$title', '$timestamp', '$ip', 0, $user_id, $store_id, $item_id, '$item_type', '$action_type');");
...
function yourls_add_new_link( $url, $keyword = '', $title = '', $action_type = '', $store_id = 0, $item_id = 0, $item_type = '', $user_id = 0 ) {
	global $ydb;
	...
	if( !$url_exists || yourls_allow_duplicate_longurls() ) {
		...
		if ( $keyword ) {
			...
			if ( !yourls_keyword_is_free($keyword) ) {
				...
				// all clear, store !
				yourls_insert_link_in_db( $url, $keyword, $title, $action_type, $store_id, $item_id, $item_type, $user_id );
		...
		// Create random keyword	
			...
			do {
				...
				$add_url = @yourls_insert_link_in_db( $url, $keyword, $title, $action_type, $store_id, $item_id, $item_type, $user_id );
				$ok = ($free && $add_url);
...
// Return array of all informations associated with keyword. Returns false if keyword not found. Set optional $use_cache to false to force fetching from DB
function yourls_get_keywords_by_item( $item_id, $item_type )
{
  $table = YOURLS_DB_TABLE_URL;

	return yourls_get_keywords_by_custom_criteria("SELECT keyword FROM `$table` WHERE `item_id` = $item_id AND item_type = '$item_type'");
}

function yourls_get_keywords_by_store( $store_id )
{
  $table = YOURLS_DB_TABLE_URL;

	return yourls_get_keywords_by_custom_criteria("SELECT keyword FROM `$table` WHERE `store_id` = $store_id");
}

function yourls_get_keywords_by_custom_criteria($query)
{
  global $ydb;
	if( isset( $ydb->infos[$keyword] ) && $use_cache == true ) {
