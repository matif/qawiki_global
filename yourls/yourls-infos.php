...
$other_info = false;
if (isset($_REQUEST['item_id']) && isset($_REQUEST['item_type']))
{
  $other_info = true;
}
elseif (isset($_REQUEST['store_id']))
{
  $other_info = true;
}


if ( !isset( $keyword ) && !$other_info ) {
...
if( yourls_do_log_redirect() ) {
  ...
  if(!$aggregate) {
    if(isset($_REQUEST['item_id']) && isset($_REQUEST['item_type'])) {
      ...
      $keyword_list = yourls_get_keywords_by_item($item_id, $item_type);
      $aggregate = true;
    } elseif(isset($_REQUEST['store_id'])) {
      $store_id = $_REQUEST['store_id'];
      $keyword_list = yourls_get_keywords_by_store($store_id);
      $aggregate = true;
    }
  } else {
		...
		<?php if ( $referrers ) { ?>
			...
			<tr>
				<td valign="top">
					...
					yourls_stats_pie( $referrer_sort, 5, '420x220' );
				...
				<td valign="top">
					...
					<?php
					yourls_stats_pie( array('Direct'=>$direct, 'Referrers'=> $notdirect), 5, '420x220' );
					?>
