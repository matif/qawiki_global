    ...
    var methods = {
        ...
        attach: function(userOptions) {
            ...
            if (!options.binded) {
                	...
                	// bind form.submit
					//$("#register_button").bind("click",methods._onClickEvent(form));
					//$("#register_button").click(function(){methods._onClickEvent(form)});
...
*/
        ...
        _validateField: function(field, options, skipAjaxFieldValidation) {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "test":
					// alert('HERE IN CASE TEST');
					 required = true;
					 errorMsg = methods._required_not(field, rules, i, options);	
        ...
        _required: function(field, rules, i, options) {
		   ...
		   _required_not: function(field, rules, i, options) {
			   //alert('REQUIRED NOT');
            switch (field.attr("type")) {
                case "text":
                case "password":
                ...
                default:
					...
					if(name == 'first_name')
					{
						if (!field.val() || field.val() == 'First Name' )
						{
							return options.allrules[rules[i]].alertText+' First Name';
						}		 
					}else if(name == 'last_name')
					{
						if (!field.val() || field.val() == 'Last Name' )
						{
							return options.allrules[rules[i]].alertText+' Last Name';
						}	
					}else if(name == 'username')
					{
						if (!field.val() || field.val() == 'User Name' )
						{
							return options.allrules[rules[i]].alertText+' User Name';
						}	
					}else if(name == 'password')
					{
						if (!field.val() || field.val() == 'Password' )
						{
							return options.allrules[rules[i]].alertText+' Password';
						}
					}else if(name == 'email')
					{
						if (!field.val() || field.val() == 'Email Address' )
						{
							return options.allrules[rules[i]].alertText+' Email Address';
						}
					}else if(name == 'captcha')
					{
						if (!field.val() || field.val() == 'Captcha' )
						{
							return options.allrules[rules[i]].alertText+' Captcha';
						}
					}
                    break;
                case "radio":
